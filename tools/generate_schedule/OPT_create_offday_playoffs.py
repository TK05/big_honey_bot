...
PLAYOFFS_START_DATE = date(2026, 4, 18) # Day playoffs start
PLAYOFFS_END_DATE = date(2026, 6, 19) # Day playoffs end (IE: game 7 of finals)
...
def create_schedule(start, end, playoffs=True, count_down=False):
    ...
    for d in range((end - now.date()).days):
        ...
        post_time_dict = {
            ...
            "day": post_date.day,
            "hour": int(setup['pre_game_post_hour']),
            "minute": int(0)
        }
